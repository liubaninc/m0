...
import (
	...
	"github.com/liubaninc/m0/x/permission/client/cli"
...
func networkWithAccountObjects(t *testing.T, n int) (*network.Network, []*types.Account) {
	...
	for i := 0; i < n; i++ {
		state.AccountList = append(state.AccountList, &types.Account{Creator: "ANY", Index: strconv.Itoa(i)})
	}
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.Equal(t, tc.obj, resp.Account)
			}
		})
	}
}
...
func TestListAccount(t *testing.T) {
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			for j := i; j < len(objs) && j < i+step; j++ {
				assert.Equal(t, objs[j], resp.Account[j-i])
			}
		}
	})
	...
	t.Run("Total", func(t *testing.T) {
		...
		require.Equal(t, objs, resp.Account)
	})
}
