...
bool XRC01::authorization(const std::string& to, uint64_t token_id,
    ...
    mchain::Account account = _ctx->sender();
    ...
    if (!mchain::safe_assert(asset_info.amount() >= asset_info.authorized())) {
    ...
    // Update asset_info
    if (!mchain::safe_assert(asset_info.authorized() + amount >= amount)) {
        printf("safemath assert failed");
        return false;
    }
    ...
    if (!_authorize_info.find({{"id", authorize_info_id}}, &authorize_info)) {
        ...
        if (!mchain::safe_assert(amount + authorize_info.amount() >=
...
bool XRC01::withdraw_authorization(const std::string& to, uint64_t token_id,
    ...
    if (!mchain::safe_assert(asset_info.authorized() >= amount)) {
        printf("safemath assert failed");
        return false;
    }
    asset_info.set_authorized(asset_info.authorized() - amount);
    ...
    if (!mchain::safe_assert(authorize_info.amount() >= amount)) {
        printf("safemath assert failed");
        return false;
    }
...
bool XRC01::transfer(const std::string& to, uint64_t token_id,
    ...
    if (!mchain::safe_assert(asset_info_caller.amount() >=
    ...
    if (!mchain::safe_assert(asset_info_caller.amount() >= amount)) {
        printf("safemath assert failed");
        return false;
    }
    ...
    if (!_asset_info.find({{"id", asset_info_to_id}}, &asset_info_to)) {
        ...
        if (!mchain::safe_assert(asset_info_to.amount() + amount >= amount)) {
            printf("safemath assert failed");
            return false;
        }
...
bool XRC01::transfer_from(const std::string& from, const std::string& to,
    ...
    if (!mchain::safe_assert(asset_info_from.amount() >= amount)) {
        printf("safemath assert failed");
        return false;
    }
    asset_info_from.set_amount(asset_info_from.amount() - amount);

    if (!mchain::safe_assert(asset_info_from.authorized() >= amount)) {
        printf("safemath assert failed");
        return false;
    }
