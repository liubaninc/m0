...
func newContractProcess(cfg *NativeConfig, name, basedir, chainAddr string, desc *xmodel.WasmCodeDesc) (*contractProcess, error) {
	process := &contractProcess{
		...
		logger:        log.NewTMLogger(os.Stdin).With("contract", name),
	}
	return process, nil
}
...
func (c *contractProcess) GetDesc() *xmodel.WasmCodeDesc {
	return c.desc
}
