...
func codeDescEqual(a, b *xmodel.WasmCodeDesc) bool {
	return bytes.Equal(a.GetDigest(), b.GetDigest())
}

func (c *codeManager) lookupMemCache(name string, desc *xmodel.WasmCodeDesc) (*contractCode, bool) {
	c.mutex.Lock()
...
func (c *codeManager) makeMemCache(name, libpath string, desc *xmodel.WasmCodeDesc) (*contractCode, error) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
...
func (c *codeManager) lookupDiskCache(name string, desc *xmodel.WasmCodeDesc) (string, bool) {
	...
	var localDesc xmodel.WasmCodeDesc
...
func (c *codeManager) makeDiskCache(name string, desc *xmodel.WasmCodeDesc, codebuf []byte) (string, error) {
...
// not used now
func makeCacheId(desc *xmodel.WasmCodeDesc) string {
	h := sha1.New()
