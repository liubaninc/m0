...
func DecodeX509Certificate(pemCertificate string) (*X509Certificate, error) {
	...
	if block == nil {
		return nil, sdkerrors.Wrapf(sdkerrors.ErrUnknownRequest, "Could not decode pem certificate")
	}
...
func (c X509Certificate) Verify(parent *X509Certificate) error {
	...
	if _, err := c.Certificate.Verify(opts); err != nil {
		return sdkerrors.Wrapf(sdkerrors.ErrUnknownRequest, "Certificate verification failed. Error: %v", err)
	}
