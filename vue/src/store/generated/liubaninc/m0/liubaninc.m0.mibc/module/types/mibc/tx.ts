...
const baseMsgSendIbcUTXO: object = { sender: '', port: '', channelID: '', timeoutTimestamp: 0, receiver: '', amount: '' }

export const MsgSendIbcUTXO = {
  encode(message: MsgSendIbcUTXO, writer: Writer = Writer.create()): Writer {
    if (message.sender !== '') {
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgSendIbcUTXO {
    ...
    const message = { ...baseMsgSendIbcUTXO } as MsgSendIbcUTXO
...
const baseMsgSendIbcUTXOResponse: object = {}

export const MsgSendIbcUTXOResponse = {
  encode(_: MsgSendIbcUTXOResponse, writer: Writer = Writer.create()): Writer {
    return writer
  },
  ...
  decode(input: Reader | Uint8Array, length?: number): MsgSendIbcUTXOResponse {
    const reader = input instanceof Uint8Array ? new Reader(input) : input
    let end = length === undefined ? reader.len : reader.pos + length
    const message = { ...baseMsgSendIbcUTXOResponse } as MsgSendIbcUTXOResponse
    while (reader.pos < end) {
      const tag = reader.uint32()
      switch (tag >>> 3) {
        default:
