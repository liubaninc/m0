        ...
        <div class="login-main-warpper">
          ...
          <div class="main-wp-right">
            <div class="wp-right-title">
              欢迎注册 <span class="wp-name">{{ pageContext.logoName }}</span>
            </div>
            <div class="wp-right-form">
              ...
              <div class="form-row">
                <div class="right-form-name"></div>
                <div class="right-form-input login-input">
...
<script>
import { getImgCode, registerUser } from "@/server/user/login";
...
export default {
  ...
  methods: {
    ...
    async register() {
      ...
      if (!/^[a-zA-Z0-9]{4,30}$/gi.test(username)) {
        this.$message.error("请输入4位以上字母或数字用户名");
        return;
      }

      if (!/^[a-zA-Z0-9]{6,30}$/gi.test(pwd)) {
        this.$message.error("请输入6位以上字母或数字密码");
        return;
      }
      // if (!username) {
      ...
      let regRes = await registerUser.call(this, {
        name: username,
        password: pwd,
      ...
      if (regRes) {
        localCache.set("loginUser", regRes);
