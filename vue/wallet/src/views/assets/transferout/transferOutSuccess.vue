  ...
  <div class="wallet">
    <div class="wallet-warpper">
      <el-breadcrumb separator="/" class="wallet-breadcrumb">
        <el-breadcrumb-item :to="{ path: `/assets?address=${address}` }"
        ...
        <div class="wallet-form wallet-form-mesg">
          ...
          <div class="wallet-form-info">
            ...
            <div class="form-info-row">
              ...
              <template v-if="trxDetail.coins && trxDetail.coins.length">
                <div class="cm-text-overflow info-row-address">
                  {{ trxDetail.coins[0].denom }}
                </div>
              </template>
              <template v-else>
                <div class="cm-text-overflow info-row-address"></div>
              </template>
            </div>
            <div class="form-info-row">
              <div class="info-row-name">
                <!-- <template
                  v-if="trxDetail.coins && trxDetail.coins[0].amount < 0"
                  >转出</template
                >
                <template
                  v-else="trxDetail.coins && trxDetail.coins[0].amount > 0"
                  >转入</template
                >总数量 -->
                {{ trxDetail.type }}总量
              </div>
              <div
                ...
                v-if="trxDetail.coins"
              >
                {{ trxDetail.coins[0].amount | formateAmount }}
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  created() {
    let { hash, coin } = this.$route.query;
    let { address } = localCache.get("wallet");
    if (hash && address) {
      this.address = address;
      this.coin = coin;
      this.getTrxDetailByHash(hash);
    }
  },
  methods: {
    async getTrxDetailByHash(hash) {
      let trxDetail = await queryTrxDetail({
        hash,
