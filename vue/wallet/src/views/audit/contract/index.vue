<template>
  <div class="com-breadcrumb trx">
    ...
    <div class="trx-list">
          ...
          <div class="cm-module-bg trx-list-row" :key="peer.hash">
            <div class="list-row-top">
              ...
              <!-- <div class="top-assets">
                ...
                <div class="assets-name">{{ peer.object || '---' }}</div>
              </div> -->
      ...
      <template v-else>
        <div class="cm-module-bg trx-list-row trx-no-data">暂无合约数据</div>
      </template>
...
<script>
import { queryContracts } from '@/server/audit'
export default {
  data() {
    return {
      actionValue: '全部',
  ...
  methods: {
    ...
    async getPeers({ action, address, pageNum = 1, pageSize = 10 }) {
      let { page_num, page_size, total, items } = await queryContracts({
        address,
        action,
        page_size: pageSize,
        page_num: pageNum,
