<template>
  <div class="com-breadcrumb trx">
    ...
    <div class="trx-list">
      ...
      <template v-else>
        <div class="cm-module-bg trx-list-row trx-no-data">暂无节点数据</div>
      </template>
...
<script>
import { queryPeers } from '@/server/audit'
export default {
  data() {
    return {
      actionValue: '全部',
  ...
  methods: {
    ...
    async getPeers({ action, address, pageNum = 1, pageSize = 10 }) {
      let { page_num, page_size, total, items } = await queryPeers({
        action,
        address,
        page_size: pageSize,
        page_num: pageNum,
