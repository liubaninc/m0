...
function issue() {
  ...
  after_input_token_from=$(m0d query utxo list-input $(m0d keys show $from -a) $coin --output json)
  after_balance_token_to=$(m0d query bank balances $(m0d keys show $to -a) --denom $coin --output json)
  after_input_token_to=$(m0d query utxo list-input $(m0d keys show $to -a) $coin --output json)
