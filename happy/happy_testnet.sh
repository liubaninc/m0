...
nodeKey=$(md5 $home/config/node_key.json | awk '{print $4}')
validatorKey=$(md5 $home/config/node_key.json | awk '{print $4}')
rm -rf $home

# more
echo "$mnemonic" | m0d init node0 --recover --home $home 
nodeKey2=$(md5 $home/config/node_key.json | awk '{print $4}')
validatorKey2=$(md5 $home/config/node_key.json | awk '{print $4}')
rm -rf $home
...
genesis=$(md5 $outputDir/node0/.m0/config/genesis.json | awk '{print $4}')
rm -rf $outputDir

m0d testnet --num $num --genesis-time $genesisTime --chain-id $chain --node-validator-key "$mnemonic" --output-dir $outputDir 
genesis2=$(md5 $outputDir/node0/.m0/config/genesis.json | awk '{print $4}')
rm -rf $outputDir
