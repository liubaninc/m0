...
m0d collect-gentxs
sed -i --in-place=.bak 's/enabled = false/enabled = true/g' ~/.m0/config/app.toml
sed -i --in-place=.bak 's/prometheus-retention-time = 0/prometheus-retention-time = 10/g' ~/.m0/config/app.toml
sed -i --in-place=.bak 's/timeout_broadcast_tx_commit = "10s"/timeout_broadcast_tx_commit = "60s"/g' ~/.m0/config/config.toml
sed -i --in-place=.bak 's/prometheus = false/prometheus = true/g' ~/.m0/config/config.toml
